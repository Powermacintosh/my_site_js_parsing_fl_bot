...
def get_all_links(html):
    ...
    try:
        web = soup.find('div', class_="b-layout__txt b-layout__txt_padbot_20").find_all('br')
        all_web = []
        for index, i in enumerate(web):
            if index == 0:
                p1 = i.find_previous_sibling(text=True)
                all_web.append(p1)
                # print('+', index, '+', p1)
            p = i.find_next_sibling(text=True)
            all_web.append(p)
            # print('+', index, '+', p)

        my_web = '\n'.join(all_web)
        if my_web == '':
            my_web = soup.find('div', class_="b-layout__txt b-layout__txt_padbot_20").text
        my_web_11 = my_web
    except:
        my_web_11 = '**----------------------------**'
    # ----------------------------------------------------------------------------------------
    try:
        my_date = soup.find('div', class_="b-layout__txt b-layout__txt_padbot_30").text.strip()
    except:
        my_date = '--Дата публикации отсутвтвует--'
    # ----------------------------------------------------------------------------------------
    try:
        my_price = soup.find('span', class_="b-layout__bold").text.strip()
    except:
        my_price = '--Price отсутвтвует--'
    # ----------------------------------------------------------------------------------------
    
    data = {'my_web_11': my_web_11,
            'my_date': my_date,
            'my_price': my_price}
...
def get_urls_pars(html):
    ...
    for index, i in enumerate(web):
        ...
        ref_link_n = refind_link(link)
        ref_link_new = ref_link_n.values()
        for index, i in enumerate (ref_link_new):
            if index == 0:
                ref_link = i
                # print(index, i)
            if index == 1:
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for index, i in enumerate(range(1, 21)): # max 300
